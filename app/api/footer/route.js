...
// Build the JS payload we serve at /footer.js (as plain JS text)
function footerJS(){
  const d = {
  ...
  // Use a single backticked JS string, and build the HTML with single-quoted strings + concatenation.
  return `(()=>{try{
    const s = document.currentScript;
    const cfg = {
      brokerage: s?.dataset?.brokerage || ${JSON.stringify(d.brokerage)},
      team:      s?.dataset?.team      || ${JSON.stringify(d.team)},
      agent:     s?.dataset?.agent     || ${JSON.stringify(d.agent)},
      office:    s?.dataset?.office    || ${JSON.stringify(d.office)},
      email:     s?.dataset?.email     || ${JSON.stringify(d.email)},
      work:      s?.dataset?.work      || ${JSON.stringify(d.work)},
      cell:      s?.dataset?.cell      || ${JSON.stringify(d.cell)},
      site:      s?.dataset?.site      || ${JSON.stringify(d.site)}
    };
...
export async function GET(request){
  ...
  return new Response(footerJS(), {
    status: 200,
    headers: { 'Content-Type':'application/javascript; charset=utf-8', ...cors(origin) }
  });
}
